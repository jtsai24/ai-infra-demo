...
output "vllm_test_inference_cmd" {
  ...
  value       = <<-EOT
    kubectl exec -n default deploy/vllm -- curl -s http://localhost:8000/v1/completions \
      ...
      -d '{"model": "qwen-7b", "prompt": "Hello", "max_tokens": 16}'
  EOT
}
...
output "grafana_password_cmd" {
  description = "Get the Grafana admin password"
  value       = "kubectl get secret -n default grafana -o jsonpath='{.data.admin-password}' | base64 --decode"
}